...
public:
	...
	struct {
		...
		bool m_splash_bot = false;
		float m_splash_radius = false;
		bool m_dynamic_sim = false;
	} m_projectile_aim;
	...
	struct {
		...
		bool m_player_local_only = false;
	...
	struct {
		struct {
			bool m_masterS{ false };
			bool m_anti_backstab{ false };
			bool m_logic_resolver{ false };
			bool m_minigun_crouch{ false };
			bool m_leg_jitter{ false };

			bool m_pitch_master{ false };
			bool m_yaws_master{ false };
			bool m_rYaw_master{ false };
			bool m_fYaw_master{ false };
		} m_anti_aim;
		...
		struct {
			bool smartYawMove1{ false };
			bool smartYawStand1{ false };
		} m_aa_sYaw;

		struct {
			bool m_fakelag{ false };
			bool m_lag_comp{ true };
			bool dynFakeLag{ false };
	...
	struct {
		...
		color m_ui_accent = color(61, 191, 227);
		color m_ui_text = color(255, 255, 255);
		color m_aimbot_fov = color(255, 255, 255);
		color m_ui_text_selected = color(61, 191, 227);
		color m_ui_text_hover = color(100, 150, 200);
		color m_ui_red = color(255, 0, 0);
		color m_ui_blue = color(61, 191, 227);
		color m_ui_yellow = color(255, 255, 0);
		color m_ui_white = color(255, 255, 255);
	} m_colors;
