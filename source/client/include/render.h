...
public:
	...
	void draw_circle(int x, int y, float radius, int segments, const color& clr)
	{
	...
	void draw_rectangle(int x, int y, int width, int height, const color& col) {
		ctx->m_surface->draw_set_color(col);
		ctx->m_surface->draw_filled_rect(x, y, x + width, y + height);
	}
