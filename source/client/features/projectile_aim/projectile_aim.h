...
private:
	...
	bool SolveProjectile(c_base_combat_weapon* weapon, const vector& start, const vector& point, float& solvedTime);
	bool HandleSplashBot(c_base_player* target, const vector& start, const vector& position, float& timeOut);
	bool IsRocketLauncher(int weaponId);
	std::vector<vector> ComputeSphere(const vector& center, float radius, int numSamples);
