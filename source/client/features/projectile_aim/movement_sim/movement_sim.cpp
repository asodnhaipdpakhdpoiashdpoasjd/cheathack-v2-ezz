...
vector c_movement_sim::run_tick() {
	...
	if (config->m_projectile_aim.m_turn_prediction) {
		float turnDelta = m_turn_speed * TICK_INTERVAL;

		if (!(m_player->m_flags() & FL_ONGROUND)) {
			// air turn
			m_move_data.m_view_angles.m_y = math::normalize_angle(m_move_data.m_view_angles.m_y + turnDelta);

			// side move for air turn
			m_move_data.m_forward_move = 0.0f;
			m_move_data.m_side_move = turnDelta < 0.0f ? -450.0f : 450.0f;

			// forward move based on velocity
			m_move_data.m_forward_move += m_player->m_velocity().length() * TICK_INTERVAL;
		}
		else {
			// ground turn
			m_move_data.m_view_angles.m_y += turnDelta;
			...
			// checks moves
			m_move_data.m_forward_move *= (1.0f - TICK_INTERVAL * 10.0f);
			m_move_data.m_side_move *= (1.0f - TICK_INTERVAL * 10.0f);
		}
	}
...
void c_movement_sim::draw_path() {
	...
	for (auto i = 1u; i < m_positions.size(); i++) {
		...
		int alpha = static_cast<int>(math::remap_val_clamped(ctx->m_engine_client->time() - m_time_stamp, 4.0f, 5.0f, 255.0f, 0.0f));

		for (int x_offset = -1; x_offset <= 1; ++x_offset) {
			for (int y_offset = -1; y_offset <= 1; ++y_offset) {
				if (x_offset == 0 && y_offset == 0) continue;
				render->line(start_screen.m_x + x_offset, start_screen.m_y + y_offset, end_screen.m_x + x_offset, end_screen.m_y + y_offset, color(0, 0, 0, alpha));
			}
		}

		render->line(start_screen.m_x, start_screen.m_y, end_screen.m_x, end_screen.m_y, color(61, 191, 227, alpha));

		if ((i % 5) == 0) {
			vector rotated = math::get_rotated_position(end, math::direction_to_angles(start - end).m_y + 90.0f, 10.0f);
			vector rotated_screen;
			...
			for (int x_offset = -1; x_offset <= 1; ++x_offset) {
				for (int y_offset = -1; y_offset <= 1; ++y_offset) {
					if (x_offset == 0 && y_offset == 0) continue;
					render->line(end_screen.m_x + x_offset, end_screen.m_y + y_offset, rotated_screen.m_x + x_offset, rotated_screen.m_y + y_offset, color(0, 0, 0, alpha));
				}
			}

			render->line(end_screen.m_x, end_screen.m_y, rotated_screen.m_x, rotated_screen.m_y, color(61, 191, 227, alpha));
		}
	}
}