...
constexpr float kEnergyRingSpeed = 1200.0f;
constexpr float kMaxVelocity = 1000000.0f;
constexpr float kMaxAngularVelocity = 1000000.0f;
constexpr float kStep = 10.0f;
constexpr float kChargeTimeThreshold = 0.1f;
...
struct Player2 {
    Vector2 velocity;
    std::vector<float> absAngles;
};

void Normalize2DInPlace(Vector2& vec) {
...
bool c_projectile_sim::init(const projectile_info_t& info) {
    ...
    float drag = 0.0f;
    vector drag_basis, angular_drag_basis;
    ...
    float max_velocity = 1000000.0f;
    float max_angular_velocity = 1000000.0f;
    ...
    case TF_PROJECTILE_CANNONBALL:
        ...
        max_angular_velocity = kMaxAngularVelocity;
        break;
    default:
        break;
    }
...
bool ShouldShootStraightDemoman(const vector& start, const vector& eyeAngles, float max_distance) {
    if (ctx->m_local_player && ctx->m_local_player->m_player_class() == CLASS_DEMOMAN) {
        vector direction;
        math::angle_vectors(eyeAngles, &direction);
        vector end = start + direction * max_distance;
        ray_t ray;
        ray.init(start, end);
        trace_t trace;

        if (ctx->m_engine_trace) {
            ctx->m_engine_trace->trace_ray(ray, MASK_SHOT_HULL, nullptr, &trace);
            return !trace.did_hit() || (trace.m_fraction > 0.99f);
        }
...
bool ShouldLobProjectileDemoman(c_base_player* player, c_base_combat_weapon* weapon) {
    if (player && weapon && player->m_player_class() == CLASS_DEMOMAN) {
        const float chargeBeginTime = weapon->m_charge_begin_time();
        const float currentTime = static_cast<float>(player->m_tick_base()) * TICK_INTERVAL;

        return chargeBeginTime > 0.0f && currentTime - chargeBeginTime > kChargeTimeThreshold;
    }
...
bool c_projectile_sim::get_info(c_base_player* player, c_base_combat_weapon* weapon, const vector& angles, projectile_info_t& info) {
    ...
    case TF_WEAPON_THEORIGINAL: {
        if (weapon->m_item_definition_index() == Soldier_m_TheOriginal)
            tf2::get_projectile_fire_setup(player, angles, vector(0.0f, 0.0f, 0.0f), position, angle, false);
        else
            tf2::get_projectile_fire_setup(player, angles, vector(23.5f, 12.0f, ducking ? 8.0f : -3.0f), position, angle, false);
...
bool c_projectile_sim::is_energy_weapon(int item_def_index) {
    return (item_def_index == Soldier_s_TheRighteousBison ||
        item_def_index == Engi_m_ThePomson6000 ||
        item_def_index == Soldier_m_TheCowMangler5000);
}
