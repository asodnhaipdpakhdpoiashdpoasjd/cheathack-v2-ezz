...
#include "../projectile_aim/movement_sim/movement_sim.h"
...
void c_esp::render_players() {
	...
	for (const auto& player : ctx->players) {
		...
		if (!player->is_enemy() && (config->m_esp.m_player_enemy || (!player->is_local_player())) && !player->is_player_on_friend_list())
			continue;
		...
		if (config->m_esp.m_player_health) {
			...
			render->outlined_rect(bounding_box.m_x - 5, render_height, 2, health_bar_height, color(0, 0, 0, 245));
			...
			wchar_t health_text[32];
			swprintf(health_text, 32, L"%i/%i", player->m_health(), player->get_max_health());

			int text_width, text_height;
			ctx->m_surface->get_text_size(ctx->m_tahoma_bold.m_font, health_text, text_width, text_height);

			int text_x = bounding_box.m_x + (bounding_box.m_w - text_width) / 2;
			int text_y = bounding_box.m_y + bounding_box.m_h + 2;

			render->outlined_text(text_x, text_y, ALIGN_DEFAULT, ctx->m_tahoma_bold.m_font, color(255, 255, 255), health_text);
		}
		...
		if (config->m_esp.m_player_flags) {
			...
			if (player->is_dormant()) {
				...
				text_offset += ctx->m_tahoma_narrow.m_tall;
			}
			...
			if (player->is_covered_in_milk()) {
				render->outlined_text(bounding_box.m_x + bounding_box.m_w + 3, y + text_offset, ALIGN_DEFAULT, ctx->m_small_fonts.m_font, color(255, 255, 255), "CUMMED ON");
				text_offset += ctx->m_tahoma_narrow.m_tall;
			}
		...
		if (config->m_esp.m_player_name) {
			...
			if (ctx->m_engine_client->get_player_info(player->entindex(), &info)) {
				float textX = bounding_box.m_x + (bounding_box.m_w * 0.5f);
				float textY = bounding_box.m_y - ctx->m_verdana_narrow.m_tall - 12;

				if (player->is_player_on_friend_list())
					render->outlined_text(textX, textY, ALIGN_CENTERHORIZONTAL, ctx->m_tahoma_bold.m_font, color(0, 200, 255), "FRIEND");
				else if (tf2::is_a_cheater_unoptimized(info.m_friends_id))
					render->outlined_text(textX, textY, ALIGN_CENTERHORIZONTAL, ctx->m_tahoma_bold.m_font, color(255, 50, 50), "CHEATER");
				else if (cheat_detection->m_suspects[player->entindex()].m_marked)
					render->outlined_text(textX, textY, ALIGN_CENTERHORIZONTAL, ctx->m_tahoma_bold.m_font, color(255, 50, 50), "SUSPECT");

				render->outlined_text(textX, textY + ctx->m_verdana_narrow.m_tall + 2, ALIGN_CENTERHORIZONTAL, ctx->m_verdana_narrow.m_font, dormant_color, util::utf_to_wide(info.m_name).c_str());
			}
		}
...
void c_esp::render_player_arrow(c_base_player* player, color clr) {
	...
	if (config->m_esp.m_arrows_esp) {
		...
		if (config->m_esp.m_player_health) {
			const int healthIndex = player->entindex();
			float& healthAnim = player_health[healthIndex];

			healthAnim = std::clamp(healthAnim, 0.0f, static_cast<float>(player->get_max_health()));

			if (healthAnim > player->m_health()) {
				const float reductionRate = (player->get_max_health() < 451) ? player->get_max_health() * 2 : player->get_max_health() * 2 * player->get_max_health() / 300.f;
				healthAnim -= reductionRate * ctx->m_globals->m_frame_time;
			}
			else {
				healthAnim = player->m_health();
			}

			const float healthFraction = math::get_fraction(healthAnim, static_cast<float>(player->get_max_health()), 0.0f);
			const float healthBarHeight = static_cast<float>(arrow_length + 4) * healthFraction;
			const float renderHeight = y - 14 + (arrow_length + 4 - healthBarHeight) + ((healthAnim < player->get_max_health()) ? 1 : 0);

			render->filled_rect(x - 16 - 2 - 1, y - 14 - 1, 4, arrow_length + 4 + 2, color(0, 0, 0, 180));

			render->filled_rect(x - 16 - 2, renderHeight, 2, healthBarHeight, tf2::get_health_color(player->m_health(), player->get_max_health()));

			if (player->m_health() < player->get_max_health()) {
				render->outlined_text(x - 16 - 2 - 1, renderHeight - 3, ALIGN_RIGHT, ctx->m_verdana_narrow.m_font, color(255, 255, 255), L"%i", player->m_health());
			}
		}

		if (config->m_esp.m_player_weapon && player->get_active_weapon()) {
