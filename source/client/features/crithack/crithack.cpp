...
void c_crithack::reset_crit_data() {
    m_crit_dmg = 0;
    ...
    m_old_player_resource_damage = ctx->m_player_resource->get_damage(ctx->m_local_player->entindex());
    m_melee_dmg = 0;
    m_dmg_done_weapon = 0;
}
...
bool c_crithack::find_crit_command(c_base_combat_weapon* weapon, c_user_cmd* cmd) {
    ...
    float old_crit_token_bucket = weapon->m_crit_token_bucket();
    m_is_going_through_crit_cmds = true;
...
void c_crithack::run_crit_indicator_calc() {
    ...
    if (m_wait_for_resync && m_old_player_resource_damage < player_resource_dmg) {
        ...
        if (m_added_crit_dmg) {
            m_crit_dmg = m_crit_dmg - m_last_damage_change + damage_diff;
            m_added_crit_dmg = false;
        }
        else {
            m_crit_dmg = m_crit_dmg + m_last_damage_change - damage_diff * 3;
        }
    }
    ...
    if (ctx->m_local_player->m_damage_done() > m_old_dmg) {
        if (ctx->m_local_weapon->get_slot() != e_weapon_slots::SLOT_MELEE) {
            ...
            m_melee_dmg += ctx->m_local_player->m_damage_done() - m_old_dmg;
        }
    ...
    if (ctx->m_local_weapon->get_slot() != e_weapon_slots::SLOT_MELEE) {
        if (ctx->m_local_weapon->get_weapon_data().m_use_rapid_fire_crits) {
            ...
            m_official_crit_dmg = m_crit_dmg + ctx->m_local_player->m_crit_mult() * 3 - 1000;
        }
        else {
            int crit_multiplier = 3, weapon_damage = ctx->m_local_weapon->get_weapon_data().m_damage, per_shot = ctx->m_local_weapon->get_weapon_data().m_bullets_per_shot, cum_bucket = ctx->m_local_weapon->m_crit_token_bucket(), base_damage = (1000 - weapon_damage);

            int full_damage = weapon_damage * per_shot;
            int critical_full_damage = full_damage * crit_multiplier;
