...
std::vector<vector> c_hitscan::generate_points_for_hitbox(c_base_player* player, int hitbox, matrix_3x4* backtrack_matrix, model_t* model) {
	...
	case CLASS_SCOUT:
		...
		break;
	case CLASS_SNIPER:
		scale = (hitbox != HITBOX_HEAD) ? config->m_aimbot.m_sniper_mp / 100.f : 1.0f;
		break;
	default:
	...
	for (const vector& point : untransformed_points) {
		...
		if (tf2::is_visible(player, nullptr, player->get_world_space_center(), transformed, MASK_SHOT | CONTENTS_GRATE)) {
...
shot_info_t c_hitscan::handle_hvh_classes(const target_t& target) {
	shot_info_t info{};
	...
	if (config->m_aimbot.m_safe_point || player->m_player_class() == CLASS_HEAVY)
		hitboxes = { 1, 2, 3, 4 ,5  };
			...
			for (auto& hitbox : hitboxes) {
				...
				for (auto& point : points) {
					if (tf2::is_visible(ctx->m_local_player, player, ctx->m_local_player->get_shoot_position(), point, MASK_SHOT | CONTENTS_GRATE)) {
						info.m_choked_packets = -1;
						info.m_entity = player;
						info.m_found = true;
						info.m_pos = point;
						info.m_sim_time = player->m_simulation_time();
						// Test
						// if (player->get_active_weapon() && player->get_active_weapon()->get_weapon_id() == TF_WEAPON_MINIGUN) {
						//     info.m_choked_packets = std::min(info.m_choked_packets, 1);
						// }
					}
				}
			}
	...
	if (!info.m_found && (config->m_exploits.m_fake_latency || config->m_exploits.m_backtrack)) {
		auto& records = backtrack->m_records[player->entindex()];
...
std::map<int, std::vector<int>> weaponHitboxes;

std::vector<int> getHitboxesForWeapon(int weaponID) {
	auto it = weaponHitboxes.find(weaponID);

	if (it != weaponHitboxes.end()) {
		return it->second;
	}

	return { HITBOX_NECK, HITBOX_PELVIS };
}

void initializeWeaponHitboxes() {
	weaponHitboxes[TF_WEAPON_MINIGUN] = { HITBOX_PELVIS, HITBOX_CHEST,  HITBOX_UPPER_CHEST, HITBOX_LEFT_CALF, HITBOX_RIGHT_CALF, };
}

shot_info_t c_hitscan::handle_other(target_t target) {
	shot_info_t info{};

	auto playerEntity = target.m_entity->as<c_base_player>();
	if (!playerEntity)
		return info;

	int activeWeaponHandle = playerEntity->m_active_weapon();
	auto activeWeapon = reinterpret_cast<c_base_combat_weapon*>(ctx->m_entity_list->get_client_entity_from_handle(activeWeaponHandle));

	if (!activeWeapon)
		return info;

	int weaponID = activeWeapon->get_weapon_id();
	std::vector<int> hitboxes = getHitboxesForWeapon(weaponID);

	// if no fake latency and if it is on and is lower than 120ms then do the original model
	if (!config->m_exploits.m_fake_latency || (config->m_exploits.m_fake_latency && config->m_exploits.m_fake_latency_amount < 120)) {
		matrix_3x4 matrix[128];
		playerEntity->setup_bones(matrix, 128, 0x100, ctx->m_globals->m_current_time);

		for (auto& hitbox : hitboxes) {
			vector pos = playerEntity->get_hitbox_position_from_matrix(hitbox, matrix, playerEntity->get_model());
			if (tf2::is_visible(ctx->m_local_player, playerEntity, ctx->m_local_player->get_shoot_position(), pos, MASK_SHOT | CONTENTS_GRATE)) {
				info.m_choked_packets = -1; // no backtrack
				info.m_entity = playerEntity;
				info.m_found = true;
				info.m_pos = pos;
				info.m_sim_time = playerEntity->m_simulation_time();
	...
	if (config->m_exploits.m_fake_latency || config->m_exploits.m_backtrack) {
		if (backtrack->m_records[playerEntity->entindex()].size() > 2) {
			for (size_t i = 2; i < backtrack->m_records[playerEntity->entindex()].size(); i++) {
				auto& rec = backtrack->m_records[playerEntity->entindex()][i];
				if (!backtrack->is_tick_valid(rec.m_simulation_time))
					continue;

				for (auto& hitbox : hitboxes) {
					vector pos = playerEntity->get_hitbox_position_from_matrix(hitbox, rec.m_bones, rec.m_model);
					if (tf2::is_visible(ctx->m_local_player, playerEntity, ctx->m_local_player->get_shoot_position(), pos, MASK_SHOT | CONTENTS_GRATE)) {
						info.m_choked_packets = rec.m_choked_packets;
						info.m_entity = playerEntity;
						info.m_found = true;
