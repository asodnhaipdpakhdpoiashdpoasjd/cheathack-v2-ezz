...
public:
	...
	void setupPitch(c_user_cmd* cmd, int pMode);
	void setupYaw(c_user_cmd* cmd, int yaw, bool rORf);
	...
	struct globals_backup {
		...
		inline void copy() {
			...
			//_current_time = ctx->m_globals->m_current_time;
