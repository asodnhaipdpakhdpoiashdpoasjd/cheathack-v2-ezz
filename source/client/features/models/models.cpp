...
void c_models::render_players() {
	...
	for (const auto& player : ctx->players) {
		if (player->is_dormant() || (!player->is_enemy() && config->m_models.m_player_enemy_only && !player->is_player_on_friend_list() && (!player->is_local_player() || config->m_models.m_player_local_only)))
			continue;
