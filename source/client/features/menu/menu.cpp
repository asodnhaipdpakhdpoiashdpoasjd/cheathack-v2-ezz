...
bool c_menu::menu_tab(std::string label, bool active) {
	...
	render->outlined_text(x + (w * 0.5f), y + 8, ALIGN_CENTER, ctx->m_tahoma_narrow.m_font, active ? config->m_colors.m_ui_text_selected : config->m_colors.m_ui_text, label.c_str());
...
// shoutout to jago for making some features here 
...
void c_menu::save_config() {
	...
	if (output.is_open()) {
		...
		Save["m_esp"]["m_player_enemy"] = config->m_esp.m_player_enemy; // m_player_local
		Save["m_esp"]["m_player_local"] = config->m_esp.m_player_local;
		...
		Save["m_models"]["m_player_local_only"] = config->m_models.m_player_local_only;
		...
		Save["m_projectile_aim"]["m_splash_bot"] = config->m_projectile_aim.m_splash_bot;
		Save["m_projectile_aim"]["m_splash_radius"] = config->m_projectile_aim.m_splash_radius;
		Save["m_projectile_aim"]["m_dynamic_sim"] = config->m_projectile_aim.m_dynamic_sim;

		/*config->m_projectile_aim.m_splash_bot = Load["m_projectile_aim"]["m_splash_bot"].asBool();
		config->m_projectile_aim.m_splash_radius = Load["m_projectile_aim"]["m_splash_radius"].asFloat();
		config->m_projectile_aim.m_dynamic_sim = Load["m_projectile_aim"]["m_dynamic_sim"].asBool();*/
		...
		Save["m_visuals"]["m_remove_recoil"] = config->m_visuals.m_remove_recoil; //m_spectatorlist
		Save["m_visuals"]["m_spectatorlist"] = config->m_visuals.m_spectatorlist;
...
void c_menu::load_config() {
	std::ifstream load("Settings.json"); // figgy.jenga
	...
	if (load.is_open()) {
		...
		config->m_esp.m_player_enemy = Load["m_esp"]["m_player_enemy"].asBool(); // m_player_local
		config->m_esp.m_player_local = Load["m_esp"]["m_player_local"].asBool();
		...
		config->m_models.m_player_local_only = Load["m_models"]["m_player_local_only"].asBool();
		...
		config->m_visuals.m_remove_recoil = Load["m_visuals"]["m_remove_recoil"].asBool();//m_spectatorlist
		config->m_visuals.m_spectatorlist = Load["m_visuals"]["m_spectatorlist"].asBool();
...
void c_menu::run() {
	...
	if (m_open || m_fade_elapsed < m_fade_time) {
		...
		enable_clipping(m_window); {
			...
			if (menu_tab("Hack vs Hack", m_current_tab == TAB_HVH))
				m_current_tab = TAB_HVH;

			if (menu_tab("Miscellaneous", m_current_tab == TAB_MISC))
			...
			case TAB_AIM: {
				...
				case SUBTAB_AIMBOT: {
					menu_group_start(false, true, true); {
						...
						if (config->m_aimbot_global.m_enabled) {
						slider_int("Aimbot FOV", config->m_aimbot_global.m_aim_fov, 0, 180, 1, "%i deg.");
						key_bind("Aim Key", config->m_aimbot_global.m_aim_key.m_key);
							multi_combo_box("Other", { { "Ignore Friends", &config->m_aimbot_global.m_ignore_friends },
								{ "Ignore Cloaked", &config->m_aimbot_global.m_ignore_cloaked },
								{ "Draw Fov", &config->m_aimbot.m_draw_fov },
								{ "Plain Aimbot", &config->m_aimbot.m_plain } });
						}
					} menu_group("Global");
					...
					menu_group_start(false, false, false); {
						...
						check_box("Anti Wrap", config->m_aimbot.m_delayshot);
						slider_int("Delay", config->m_aimbot.m_delayshot_percent, 0, 15, 1, "%i%%");
						check_box("Auto Scope", config->m_aimbot.m_auto_scope);
						check_box("Baim Lethal", config->m_aimbot.m_baim_on_lethal);
						slider_float("Heavy Scale", config->m_aimbot.m_heavy_mp, 15.f, 100.f, 1.0f, "%1.0f");
						slider_float("Scout Scale", config->m_aimbot.m_scout_mp, 15.f, 100.f, 1.0f, "%1.0f");
						slider_float("Sniper Scale", config->m_aimbot.m_sniper_mp, 15.f, 100.f, 1.0f, "%1.0f");
					...
					menu_group_start(true, true, true); {
						...
						slider_float("Sample Time", config->m_projectile_aim.m_sample_time, 0.0f, 1.0f, 0.1f, "%0.1fs");
						slider_float("Minimum Hitchance", config->m_projectile_aim.m_hitchance, 0.0f, 50.0f, 1.0f, "%.0f%%");
						slider_float("Max Simulation Time", config->m_projectile_aim.m_max_simulation_time, 0.f, 10.f, 1.0f, "%1.fs");
						check_box("Dynamic Simulation Time", config->m_projectile_aim.m_dynamic_sim);
						check_box("SplashBot", config->m_projectile_aim.m_splash_bot); // pasted from travishook
						slider_float("Splash Radius", config->m_projectile_aim.m_splash_radius, 0.0f, 10000.0f, 1.0f, "%.0f"); // pasted from travishook
				...
				case SUBTAB_AUTOMATIONS: {
					...
					menu_group_start(true, true, true); {
						...
						check_box("Auto Vaccinator", config->m_automations.m_auto_vacc_pop);
			...
			case TAB_VISUALS: {
				...
				case SUBTAB_ESP: {
					menu_group_start(false, true, true); {
						...
						check_box("Player Local Only", config->m_esp.m_player_local);
				...
				case SUBTAB_MODELS: {
					...
					menu_group_start(false, false, false); {
						...
						check_box("Player Local Only", config->m_models.m_player_local_only); 	
				...
				case SUBTAB_OUTLINES: {
					...
					menu_group_start(false, false, false); {
						...
						//check_box("Local Only", config->m_outlines.m_player_local_only);
				...
				case SUBTAB_MISC_: {
					menu_group_start(false, true, true); {
						...
						check_box("Spectator List", config->m_visuals.m_spectatorlist);
						//button("Kill Ping", [&]() { kill_ping(); });
			...
			case TAB_HVH: {
				...
				case SUBTAB_ANTIAIM: {
					menu_group_start(false, true, true); {
						check_box("Master Switch", config->m_hvh.m_anti_aim.m_masterS);
						check_box("Resolver", config->m_hvh.m_anti_aim.m_logic_resolver);
						check_box("Pitch Master Switch", config->m_hvh.m_anti_aim.m_pitch_master);
						check_box("Anti Backstab", config->m_hvh.m_anti_aim.m_anti_backstab);
						check_box("Leg Jitter", config->m_hvh.m_anti_aim.m_leg_jitter);
						if (config->m_hvh.m_anti_aim.m_pitch_master) {
							combo_box("Pitch", config->m_hvh.m_aa_pitch.pitch, { "None", "Auto", "Zero", "Up", "Down", "Fake Up", "Fake Down", "Random", "Reals Random", "Fakes Random", "Random Fake Up", "Random Fake Do-", "Up Fake Random", "Down Fake Rand-", "Jitter", "Fake Jitter", "Jitter Fake Up", "Jitter Fake Down", "Up Fake Jitter", "Down Fake Jitter", "Custom" });

							if (config->m_hvh.m_aa_pitch.pitch == 20) {

								check_box("Jitter?", config->m_hvh.m_aa_pitch.djPitch);

								if (config->m_hvh.m_aa_pitch.djPitch) {
									check_box("Triple Jitter?", config->m_hvh.m_aa_pitch.tjPitch);
								}

								if (!config->m_hvh.m_aa_pitch.djPitch) {
									slider_float("Main Pitch Angle", config->m_hvh.m_aa_pitch.mainPitch, -271.0f, 271.0f, 1);
								}

								else {
									slider_float("1st Jitter Angle", config->m_hvh.m_aa_pitch.firstJit, -271.0f, 271.0f, 1.0f);
									slider_float("2nd Jitter Angle", config->m_hvh.m_aa_pitch.secoundJit, -271.0f, 271.0f, 1.0f);

									if (config->m_hvh.m_aa_pitch.tjPitch) {
										slider_float("3rd Jitter Angle", config->m_hvh.m_aa_pitch.thirdJit, -271.0f, 271.0f, 1.0f);
									}
								}
							}
						}

						check_box("Yaws Master Switch", config->m_hvh.m_anti_aim.m_yaws_master);

						if (config->m_hvh.m_anti_aim.m_yaws_master) {
							check_box("Real Yaw", config->m_hvh.m_anti_aim.m_rYaw_master);

							if (config->m_hvh.m_anti_aim.m_rYaw_master) {
								combo_box("Real Yaw", config->m_hvh.m_aa_rYaw.rYaw, { "Forward", "Left", "Right", "Back", "Spin Left", "Spin Right", "Jitter", "Custom", "" });

								if (config->m_hvh.m_aa_rYaw.rYaw == 4 || config->m_hvh.m_aa_rYaw.rYaw == 5)
									slider_int("Real Spin Speed", config->m_hvh.m_aa_rYaw.rSS, 1, 20, 1);

							}

							check_box("Fake Yaw", config->m_hvh.m_anti_aim.m_fYaw_master);
							if (config->m_hvh.m_anti_aim.m_fYaw_master) {
								combo_box("Fake Yaw", config->m_hvh.m_aa_fYaw.fYaw, { "Forward", "Left", "Right", "Back", "Spin Left", "Spin Right", "Jitter", "Custom" });

								if (config->m_hvh.m_aa_fYaw.fYaw == 4 || config->m_hvh.m_aa_fYaw.fYaw == 5)
									slider_int("Fake Spin Speed", config->m_hvh.m_aa_fYaw.fSS, 1, 20, 1);
							}
						}
					} menu_group("Anti Aim General");
				...
				case SUBTAB_FAKELAG: {
					menu_group_start(false, true, true); {
						check_box("Master Switch", config->m_hvh.m_fake_lag.m_fakelag);
						check_box("Lag Compensation", config->m_hvh.m_fake_lag.m_lag_comp);
						check_box("Dynamic Fake Lag", config->m_hvh.m_fake_lag.dynFakeLag);
						slider_int("Max Ticks", config->m_hvh.m_fake_lag.m_max_ticks, 1, 22, 1);
						slider_int("Max Unchoke Delay", config->m_hvh.m_fake_lag.m_max_unchoke_del, 0, 15, 1);
					} menu_group("Fakelag");
					break;
				}
				}
			...
			case TAB_EXPLOITS: {
				...
				menu_group_start(false, false, false); {
					...
					slider_int("Fake Latency Amount", config->m_exploits.m_fake_latency_amount, 0, 1000, 10, "%i ms");
