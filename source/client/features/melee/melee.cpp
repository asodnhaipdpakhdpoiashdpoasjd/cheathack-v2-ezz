...
void c_melee::run(c_user_cmd* cmd)
{
	...
	for (const auto& entity : ctx->aimbot_entities) {
		...
		// predict swing
		float dst = (ctx->m_local_player->get_world_space_center() + ctx->m_local_player->m_velocity() * TICKS_TO_TIME(6)).dist_to(player->get_world_space_center());
	...
	if (target.m_player)
	{
		...
		if (ctx->m_local_player->m_player_class() == CLASS_SCOUT ||
			ctx->m_local_player->m_player_class() == CLASS_SOLDIER ||
			ctx->m_local_player->m_player_class() == CLASS_PYRO ||
			ctx->m_local_player->m_player_class() == CLASS_DEMOMAN ||
			ctx->m_local_player->m_player_class() == CLASS_HEAVY ||
			ctx->m_local_player->m_player_class() == CLASS_ENGINEER ||
			ctx->m_local_player->m_player_class() == CLASS_MEDIC ||
			ctx->m_local_player->m_player_class() == CLASS_SNIPER) {
